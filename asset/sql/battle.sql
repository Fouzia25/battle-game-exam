...
--
INSERT INTO
  `appartenirteam1` (`IdLien1`, `IdTeam1`, `Idplayer`)
VALUES
  (4, 1, 1),
  (5, 1, 2),
  (6, 1, 3);
...
--
INSERT INTO
  `appartenirteam2` (`IdLien2`, `IdTeam2`, `Idplayer`)
VALUES
  (1, 1, 4),
  (2, 1, 5),
  (3, 1, 6),
...
--
INSERT INTO
  `battles` (`IdBattle`, `IdTeam1`, `IdTeam2`)
VALUES
  (1, 1, 1);
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`localhost` SQL SECURITY DEFINER VIEW `battledata` AS
SELECT
  `battles`.`IdBattle` AS `IdBattle`,
  `appartenirteam1`.`IdLien1` AS `IdLien1`,
  `team1`.`Buff1` AS `Buff`,
  'Team1' AS `Team`,
  `player`.`Idplayer` AS `Idplayer`,
  `player`.`NamePlayer` AS `NamePlayer`,
  `player`.`Attplayer` AS `Attplayer`,
  `player`.`DefPlayer` AS `DefPlayer`,
  `player`.`LevelPlayer` AS `LevelPlayer`,
  `perso`.`NamePerso` AS `NamePerso`,
  `perso`.`imgIdlePerso` AS `imgIdlePerso`
FROM
  (
    `battles`
    join (
      (
        (
          `appartenirteam1`
          left join `team1` on(
            (`appartenirteam1`.`IdTeam1` = `team1`.`IdTeam1`)
          )
        )
        left join `player` on(
          (
            `appartenirteam1`.`Idplayer` = `player`.`Idplayer`
          )
        )
      )
      left join `perso` on((`player`.`IdPerso` = `perso`.`IdPerso`))
    )
  )
union
select
  `battles`.`IdBattle` AS `IdBattle`,
  `appartenirteam2`.`IdLien2` AS `IdLien2`,
  `team2`.`Buff2` AS `Buff`,
  'Team2' AS `Team`,
  `player`.`Idplayer` AS `Idplayer`,
  `player`.`NamePlayer` AS `NamePlayer`,
    ...
    join (
      (
        (
          `appartenirteam2`
          left join `team2` on(
            (`appartenirteam2`.`IdTeam2` = `team2`.`IdTeam2`)
          )
        )
        left join `player` on(
          (
            `appartenirteam2`.`Idplayer` = `player`.`Idplayer`
          )
        )
      )
      left join `perso` on((`player`.`IdPerso` = `perso`.`IdPerso`))
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`localhost` SQL SECURITY DEFINER VIEW `top` AS
SELECT
  `player`.`NamePlayer` AS `NamePlayer`,
  `player`.`LevelPlayer` AS `LevelPlayer`
FROM
  `player`
ORDER BY
  `player`.`LevelPlayer` DESC,
  `player`.`NamePlayer` ASC
LIMIT
  0, 55;
...
ALTER TABLE
  `appartenirteam1`
ADD
  CONSTRAINT `FK_appartenirteam1_Idplayer` FOREIGN KEY (`Idplayer`) REFERENCES `player` (`Idplayer`),
ADD
  CONSTRAINT `FK_appartenirteam1_IdTeam1` FOREIGN KEY (`IdTeam1`) REFERENCES `team1` (`IdTeam1`);
...
ALTER TABLE
  `appartenirteam2`
ADD
  CONSTRAINT `FK_appartenirteam2_Idplayer` FOREIGN KEY (`Idplayer`) REFERENCES `player` (`Idplayer`),
ADD
  CONSTRAINT `FK_appartenirteam2_IdTeam2` FOREIGN KEY (`IdTeam2`) REFERENCES `team2` (`IdTeam2`);
...
ALTER TABLE
  `battles`
ADD
  CONSTRAINT `FK_battles_IdTeam1_Appartenir1` FOREIGN KEY (`IdTeam1`) REFERENCES `team1` (`IdTeam1`),
ADD
  CONSTRAINT `FK_battles_IdTeam2_Appartenir2` FOREIGN KEY (`IdTeam2`) REFERENCES `team2` (`IdTeam2`);
